...
This audit covers the Torch Liquidation Bot v3.0.0, an autonomous keeper that scans Torch Market lending positions and liquidates underwater loans through a Torch Vault. The bot was reviewed for key safety, vault integration correctness, error handling, and dependency surface.
...
| File | Lines | Role |
|------|-------|------|
| `packages/bot/src/index.ts` | 192 | Entry point: keypair, vault check, scan loop |
...
| `packages/bot/src/utils.ts` | 23 | Formatting helpers and logger |
...
```typescript
const agentKeypair = Keypair.generate()  // index.ts:38
```

The keypair is generated at module load via `Keypair.generate()` from `@solana/web3.js`. This uses `nacl.sign.keyPair()` (Ed25519) with system entropy. The keypair is:

- **Not persisted** — exists only in runtime memory
- **Not exported** — `agentKeypair` is module-scoped, not in the public API
...
2. **Transaction signing** (`transaction.sign(agentKeypair)` at index.ts:111) — local signing only
...
```typescript
const vault = await getVault(connection, config.vaultCreator)  // index.ts:146
if (!vault) throw new Error(...)

const link = await getVaultForWallet(connection, agentKeypair.publicKey.toBase58())  // index.ts:153
...
const { transaction, message } = await buildLiquidateTransaction(connection, {
  ...
  vault: vaultCreator,  // index.ts:109
})
```
...
const scanAndLiquidate = async (
  ...
  vaultCreator: string,  // passed from main()
) => { ... }
```
...
|----------|-----------|--------------|
| `RPC_URL` | Must be set | Throws on startup |
...
- `RPC_URL` is used only for Solana RPC calls — never logged, transmitted externally, or stored
- `VAULT_CREATOR` is a public key (not sensitive)
- No `SOLANA_PRIVATE_KEY` env var is read — the bot generates its own keypair

**Verdict:** Configuration is properly validated. No sensitive data in config.
...
### L-1: Agent Keypair Regenerated on Every Restart

**Severity:** Low
**File:** `index.ts:38`
**Description:** The agent keypair is generated fresh on every startup. This means the vault authority must re-link a new wallet after every bot restart. For long-running deployments with periodic restarts, this creates operational friction.
**Impact:** Operational — requires re-linking after restart. No security impact.
**Recommendation:** Optionally accept `SOLANA_PRIVATE_KEY` to persist the agent wallet across restarts. The current behavior (fresh keypair) is the safer default.

### L-2: No Timeout on SDK Calls

**Severity:** Low
**File:** `index.ts:48-127`
...
The Torch Liquidation Bot v3.0.0 is a well-structured, minimal-surface keeper with correct vault integration and robust error handling. Key findings:

1. **Key safety is correct** — in-process `Keypair.generate()`, no key files, no key logging, no key transmission.
...
5. **No critical, high, or medium findings** — 2 low and 4 informational issues identified.
